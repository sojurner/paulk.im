...
export const Appbar: React.FC<µAppbar.Props> = props => {
  ...
  const rearrangeActiveToFront: µAppbar.Methods['arrangeActiveToFront'] = (
    acc,
    curr
  ) => {
    if (curr.path === routerHistory.state.currentPath) {
      return [curr, ...acc];
    }
  ...
  return (
    <Flex
      ...
      {...props}
    >
      {routerHistory.state.routeHistory
        .reduce(rearrangeActiveToFront, [])
        .map(ROUTE => {
